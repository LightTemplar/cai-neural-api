...
unit uvisualdenoisingtinyimagenet;
...
begin
  ...
  FBaseName := 'v1.0-'+IntToStr(NeuronMultiplier)+'-';
  ...
  if Not(FileExists(FBaseName+'denoising.nn')) then
  begin
    WriteLn('Creating auto encoder.');
    L32 := FAutoencoder.AddLayer([
    ...
    FAutoencoder.AddLayer([
      ...
      TNNetConvolution.Create(256 * NeuronMultiplier,3,1,1,1),
      TNNetUpsample.Create() //8x8
    ]);

    FAutoencoder.AddLayer( TNNetSum.Create([FAutoencoder.GetLastLayer(), L8]) );
    ...
    FAutoencoder.AddLayer( TNNetSum.Create([FAutoencoder.GetLastLayer(), L16]) );
    ...
    FAutoencoder.AddLayer( TNNetSum.Create([FAutoencoder.GetLastLayer(), L32]) );
  ...
  begin
    ...
    FAutoencoder.LoadFromFile(FBaseName+'denoising.nn');
  end;
  ...
  FFit.ClipDelta := 0.01;
  FFit.FileNameBase := FBaseName+'denoising';
...
begin
  ...
  pOutput.Copy(pInput);
  pInput.AddSaltAndPepper(1000, random(200)/100, -random(200)/100, {color=}true);
end;
...
begin
  ...
  begin
    ...
    DisplayInputImage(FTrainImages.List[ClassId].List[ImageId], 0);
    // Debug only: FTrainImages.List[ClassId].List[ImageId].AddSaltAndPepper(1000, random(200)/100, -random(200)/100, {color=}true);
    // Debug only: DisplayInputImage(FTrainImages.List[ClassId].List[ImageId], 0);
