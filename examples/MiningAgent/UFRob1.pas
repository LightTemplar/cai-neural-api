...
uses
  ...
  StdCtrls, URobMin2, ExtCtrls, Menus,
  neuralab, neuralplanbuilder, Buttons, neuralgeneric,
  neuralnetwork;
...
type
  ...
  // The Agent
  TMiningAgent = object
    LearningAndPredict: TEasyBytePredictionViaNNet;
...
begin
  LearningAndPredict := TEasyBytePredictionViaNNet.Create(
    pActionByteLen, pStateByteLen,
    32{neurons}, 0{CacheSize});
...
function TForm1.MoveNeuralAgent(var pMiningAgent: TMiningAgent; P: byte): extended;
  ...
  begin
    ...
    FPredictionErrors := FPredictionErrors +
      Round(pMiningAgent.LearningAndPredict.newStateFound(localCurrentState));
