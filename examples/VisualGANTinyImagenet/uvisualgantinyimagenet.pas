...
begin
  ...
  InputSample.RandomizeGaussian();
  InputSample.NormalizeMax(0.2);
end;
...
begin
  ...
  if Not(FileExists(FBaseName+IntToStr(FRandomDepth)+'-generative.nn')) then
  begin
...
//      TNNetCellBias.Create(),
    ...
    FGenerative.AddSeparableConv(32 * NeuronMultiplier, 5, 2, 1, 1, 0); // 16x16
    FGenerative.AddLayer([
          TNNetPad.Create(1),
          TNNetConvolution.Create(32 * NeuronMultiplier,3,0,1,1),
    //      TNNetCellBias.Create(),
    ...
    FGenerative.AddSeparableConv(32 * NeuronMultiplier, 5, 2, 1, 1, 0); // 32x32
    FGenerative.AddLayer([
          TNNetPad.Create(1),
          TNNetConvolution.Create(32 * NeuronMultiplier,3,0,1,1),
          TNNetDeMaxPool.Create(2)
    ]);
    FGenerative.AddSeparableConv(32 * NeuronMultiplier, 5, 2, 1, 1, 0); //64x64
    FGenerative.AddLayer([
          TNNetPad.Create(1),
          TNNetConvolution.Create(32 * NeuronMultiplier,3,0,1,1),
    //      TNNetCellBias.Create(),
    ...
    *)
    ...
    FGenerative.AddSeparableConv(64 * 4 * NeuronMultiplier, 3, 1, 1, 1, 0); // 8x18
    FGenerative.AddLayer([
          TNNetUpsample.Create(),
          TNNetPad.Create(1),
          TNNetConvolution.Create(64 * NeuronMultiplier,3,0,1,1)
    ]);
    FGenerative.AddSeparableConv(32 * 4 * NeuronMultiplier, 3, 1, 1, 1, 0); // 16x16
    FGenerative.AddLayer([
          TNNetUpsample.Create(),
          TNNetPad.Create(1),
          TNNetConvolution.Create(32 * NeuronMultiplier,3,0,1,1)
    ]);
    FGenerative.AddSeparableConv(32 * 4 * NeuronMultiplier, 3, 1, 1, 1, 0); // 32x32
    FGenerative.AddLayer([
          TNNetUpsample.Create(),
          TNNetPad.Create(1),
          TNNetConvolution.Create(32 * NeuronMultiplier,3,0,1,1)
    ]);
    FGenerative.AddSeparableConv(32 * 4 * NeuronMultiplier, 3, 1, 1, 1, 0); //64x64
    FGenerative.AddLayer([
          TNNetUpsample.Create(),
          TNNetPad.Create(1),
          TNNetConvolution.Create(32 * NeuronMultiplier,3,0,1,1),
  ...
  begin
    ...
    FGenerative.LoadFromFile(FBaseName+IntToStr(FRandomDepth)+'-generative.nn');
  end;
  ...
  begin
    ...
    FDiscriminator.AddLayer([
      ...
      TNNetPad.Create(1),
      TNNetConvolution.Create({Features=}256 * NeuronMultiplier,{FeatureSize=}3,{Padding=}0,{Stride=}2,{SuppressBias=}1), // downsample to 2x2
      //TNNetFullConnectReLU.Create(256),
      //TNNetFullConnectLinear.Create(2)
      TNNetPointwiseConvLinear.Create(2),
    ...
    *)
    ...
    FDiscriminator.AddLayerAfter([
      TNNetAvgPool.Create(8),
      TNNetConvolutionReLU.Create({Features=}64 * NeuronMultiplier,{FeatureSize=}3,{Padding=}0,{Stride=}1,{SuppressBias=}1), // downsample to 6x6
      ...
      TNNetConvolutionReLU.Create({Features=}256 * NeuronMultiplier,{FeatureSize=}3,{Padding=}0,{Stride=}1,{SuppressBias=}1), // downsample to 2x2
      TNNetAvgChannel.Create()
    ], 0);
...
begin
  ...
  (*
  if ClassId < 100
  then Result.O.SetClassForSoftMax(1)
  else Result.O.SetClassForSoftMax(0);
  RandomValue := Random(1000);
  if RandomValue < 1500 then
  begin
    Result.O.SetClassForSoftMax(1);
  end
  else
...
begin
  ...
  begin
    ...
    FGenerative.SaveToFile(FBaseName+IntToStr(FRandomDepth)+'-generative.nn');
