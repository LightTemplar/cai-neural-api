program CaiResNet20Cifar100Digits;
(*
...
type
  ...
  protected
    ...
    procedure GetTraining(Idx: integer; ThreadId: integer; pInput, pOutput: TNNetVolume);
    procedure GetValidation(Idx: integer; ThreadId: integer; pInput, pOutput: TNNetVolume);
    procedure GetTest(Idx: integer; ThreadId: integer; pInput, pOutput: TNNetVolume);
...
begin
  ...
  procedure TTestCNNAlgo.GetTraining(Idx: integer; ThreadId: integer; pInput,
    pOutput: TNNetVolume);
  var
    RandomIdx: integer;
    bClass: byte;
    aOutput: array[0..0] of byte;
  begin
    RandomIdx := Random(ImgTrainingVolumes.Count);
    bClass := ImgTrainingVolumes[RandomIdx].Tag;
    aOutput[0] := bClass;
    pInput.Copy( ImgTrainingVolumes[RandomIdx] );
    pOutput.CopyAsBits(aOutput,0,1);
  end;

  procedure TTestCNNAlgo.GetValidation(Idx: integer; ThreadId: integer; pInput,
    pOutput: TNNetVolume);
  var
    bClass: byte;
    aOutput: array[0..0] of byte;
  begin
    bClass := ImgValidationVolumes[Idx].Tag;
    aOutput[0] := bClass;
    pInput.Copy( ImgValidationVolumes[Idx] );
    pOutput.CopyAsBits(aOutput,0,1);
  end;

  procedure TTestCNNAlgo.GetTest(Idx: integer; ThreadId: integer; pInput,
    pOutput: TNNetVolume);
  var
    bClass: byte;
    aOutput: array[0..0] of byte;
  begin
    bClass := ImgTestVolumes[Idx].Tag;
    aOutput[0] := bClass;
    pInput.Copy( ImgTestVolumes[Idx] );
    pOutput.CopyAsBits(aOutput,0.1,0.9);
  end;
  ...
  begin
    ...
    NN.AddLayer([
      ...
      TNNetFullConnect.Create(128),
      TNNetFullConnectLinear.Create(8), // 8 bits = 1 byte.
      TNNetSigmoid.Create()
    ]);

    NN.DebugStructure();
    CreateCifar100Volumes(ImgTrainingVolumes, ImgValidationVolumes,
      ImgTestVolumes, csEncodeRGB, {Verbose=}True, {ValidationSampleSize=}2000);
    ...
    NeuralFit.FileNameBase := 'ResnetDigits-'+IntToStr(GetProcessId());
    NeuralFit.InitialLearningRate := 0.005;
    NeuralFit.LearningRateDecay := 0.01;
    NeuralFit.StaircaseEpochs := 10;
    NeuralFit.Inertia := 0.9;
    NeuralFit.L2Decay := 0.0;
    NeuralFit.EnableMonopolarHitComparison();
    //NeuralFit.MaxThreadNum := 1;
    NeuralFit.LossFn := nil;
    NeuralFit.FitLoading(NN, ImgTrainingVolumes.Count, ImgValidationVolumes.Count, ImgTestVolumes.Count, 64, 100, @GetTraining, @GetValidation, @GetTest);
    NeuralFit.Free;
    ReadLn();
    NN.Free;
    ImgTestVolumes.Free;
...
begin
  ...
  Application.Title:='CIFAR-100 Classification Example';
  Application.Run;
  Application.Free;
end.
